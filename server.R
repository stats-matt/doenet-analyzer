  ...
  #   })
    ...
    output$rid <-
      renderUI({
        ...
        j=input$numid
        if(i<=j){
          output$id1<- renderUI({textInput("id1", "Doenet ID 1")})
           i = i+1
        }else{output$id1=NULL}
        if(i<=j){
          output$id2<- renderUI({textInput("id2", "Doenet ID 2")})
          i = i+1
        }else{output$id2=NULL}
        if(i<=j){
          output$id3<- renderUI({textInput("id3", "Doenet ID 3")})
          i = i+1
        }else{output$id3=NULL}
        if(i<=j){
          output$id4<- renderUI({textInput("id4", "Doenet ID 4")})
          i = i+1
        }else{output$id4=NULL}
        if(i<=j){
          output$id5<- renderUI({textInput("id5", "Doenet ID 5")})
          i = i+1
        }else{output$id5=NULL}
      })
    ...
    df <- eventReactive(input$submit_extra | input$update, {
      if(input$submit_extra != 0){
        ...
        end_of_link = paste0("&doenetId[]=", input$id1,"&doenetId[]=", input$id2,
                      "&doenetId[]=", input$id3,"&doenetId[]=", input$id4,
                      "&doenetId[]=", input$id5)
         
      }
      else{end_of_link = ""}
      stream_in(file(paste0("https://www.doenet.org/api/getEventData.php?doenetId[]=",getQueryString()[["data"]],end_of_link
                                     )))
    })
    ...
    output$slider <-
      renderUI({
        sliderInput("maxtime", "Maximum time shown:",
                    min = 0, max = input$maxtime_set,
                    value =  c(500,10000))
      })
