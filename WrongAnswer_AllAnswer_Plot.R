...
ui <- fluidPage(
  tabPanel(
    ...
    sliderInput("integer", "Most Frequent Wrong Answers",
                min = 0, 
                max = 0,
                value = 0),
...
#==================Back-end of page and item dropdown creation==================
  ...
  observe({
    ...
    a <- cleaned_versions %>%
      ...
      filter(item == paste0("Page ", item_selected)) %>%
      filter(pageNumber == paste0("Page ", page_selected)) %>%
