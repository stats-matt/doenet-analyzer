...
output$time_plot <- renderPlot({
  cleaned() %>%
    ...
    xlim(input$times_normal[1],input$times_normal[2])
})
...
output$hist_total <- renderPlot(
  summary_data() %>%
    group_by(userId) %>%
    mutate(total = sum(itemCreditAchieved)) %>%
    ggplot(aes(x = total)) +
    geom_histogram() +
...
output$time_plot_version <- renderPlot({
  cleaned_version() %>%
    ...
    labs(x = "Time", y = "Total Credit on Page")+
    xlim(input$times_version[1],input$times_version[2])
})
...
output$hist_total_version <- renderPlot(
  summary_data_version() %>%
    ...
    summarize(total = sum(itemCreditAchieved)) %>%
    ggplot(aes(x = total)) +
    geom_histogram() +
    labs(x = "Total Points", y = "Number of Students", title = "Total Scores on Assignment")
